...
router.get('/', function(req, res, next) {
  if (req.isAuthenticated()) {
      client.query('SELECT * FROM addresses ORDER BY date_created DESC; SELECT count(*) as count FROM addresses;select * from doctors',
...
router.get('/doctors', function(req, res, next) {
    if (req.isAuthenticated()) {
        client.query('SELECT * FROM doctors ORDER BY date_created DESC; SELECT count(*) as count FROM doctors',
            function (error, results) {
                if (error) {
                    throw error;
                }
...
router.post('/doctors', isAuthenticated, function(req, res, next) {
    ...
    , (req, res) => {
        ...
        if (!errors.isEmpty()) {
            ...
            client.query(`INSERT INTO doctors (first_name, last_name, age, dob, email, phone_number, gender_id, address_id) VALUES ('${first_name}','${last_name}','${age}','${dob}','${email}','${phone_number}','${gender_id}','${address_id}')`,
                function (error, results) {
                    // error will be an Error if one occurred during the query
                    // results will contain the results of the query
                    // fields will contain information about the returned results fields (if any)
...
router.get('/doctors/:id/edit', isResource, isAuthenticated, function(req, res) {
    if (req.user.level === 1){
        client.query(`SELECT id, first_name, last_name, age, dob, email, phone_number, gender_id, address_id FROM doctors WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.put('/doctors/:id', isResource, isAuthenticated, function(req, res, next) {
    ...
    if (!errors.isEmpty()) {
        ...
        const gender_id = req.body.gender_id;
        const address_id = req.body.address_id;
        client.query(`UPDATE doctors SET first_name = '${first_name}', last_name = '${last_name}', age = '${age}', dob = '${dob}',
...
router.delete('/doctors/:id', isResource, isAuthenticated, function(req, res, next) {
    if (req.user.level === 1){
        client.query(`DELETE FROM doctors WHERE id = '${req.params.id}'`, function (error, results) {
            // error will be an Error if one occurred during the query
            // results will contain the results of the query
            // fields will contain information about the returned results fields (if any)
            if (error) {
...
router.get('/insurances', function(req, res, next) {
    if (req.isAuthenticated()) {
        client.query('SELECT * FROM insurances ORDER BY date_created DESC; SELECT count(*) as count FROM insurances',
            function (error, results) {
                if (error) {
                    throw error;
                }
...
router.post('/insurances', isAuthenticated, function(req, res, next) {
    ...
    , (req, res) => {
        ...
        if (!errors.isEmpty()) {
            ...
            const description = req.body.description;
            client.query(`INSERT INTO insurances (name, description) VALUES ('${name}','${description}')`,
                function (error, results) {
                    // error will be an Error if one occurred during the query
                    // results will contain the results of the query
                    // fields will contain information about the returned results fields (if any)
...
router.get('/insurances/:id/edit', isResource, isAuthenticated, function(req, res) {
    if (req.user.level === 1){
        client.query(`SELECT id, name, description FROM insurances WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.put('/insurances/:id', isResource, isAuthenticated, function(req, res, next) {
    ...
    if (!errors.isEmpty()) {
        ...
        const name = req.body.name;
        const description = req.body.description;
        client.query(`UPDATE insurances SET name = '${name}', description = '${description}' WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.delete('/insurances/:id', isResource, isAuthenticated, function(req, res, next) {
    if (req.user.level === 1){
        client.query(`DELETE FROM insurances WHERE id = '${req.params.id}'`, function (error, results) {
            // error will be an Error if one occurred during the query
            // results will contain the results of the query
            // fields will contain information about the returned results fields (if any)
            if (error) {
...
router.get('/medications', function(req, res, next) {
    if (req.isAuthenticated()) {
        client.query('SELECT * FROM medications ORDER BY date_created DESC; SELECT count(*) as count FROM medications',
            function (error, results, fields) {
                if (error) {
                    throw error;
                }
...
router.post('/medications', isAuthenticated, function(req, res, next) {
    ...
    , (req, res) => {
        ...
        if (!errors.isEmpty()) {
            ...
            client.query(`INSERT INTO medications (name, description, cost) VALUES ('${name}','${description}','${cost}')`,
                function (error, results) {
                    // error will be an Error if one occurred during the query
                    // results will contain the results of the query
                    // fields will contain information about the returned results fields (if any)
...
router.get('/medications/:id/edit', isResource, isAuthenticated, function(req, res) {
    if (req.user.level === 1){
        client.query(`SELECT id, name, description, cost FROM medications WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.put('/medications/:id', isResource, isAuthenticated, function(req, res, next) {
    ...
    if (!errors.isEmpty()) {
        ...
        const description = req.body.description;
        const cost = req.body.cost;
        client.query(`UPDATE medications SET name = '${name}', description = '${description}', cost = '${cost}' WHERE id = '${req.params.id}'`,
            function (error, results, fields) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.delete('/medications/:id', isResource, isAuthenticated, function(req, res, next) {
    if (req.user.level === 1){
        client.query(`DELETE FROM medications WHERE id = '${req.params.id}'`, function (error, results, fields) {
            // error will be an Error if one occurred during the query
            // results will contain the results of the query
            // fields will contain information about the returned results fields (if any)
            if (error) {
...
router.get('/patients', function(req, res, next) {
    if (req.isAuthenticated()) {
        client.query('SELECT * FROM patients ORDER BY date_created DESC; SELECT count(*) as count FROM patients',
            function (error, results, fields) {
                if (error) {
                    throw error;
                }
...
router.post('/patients', isAuthenticated, function(req, res, next) {
    ...
    , (req, res) => {
        ...
        if (!errors.isEmpty()) {
            ...
            const gender_id = req.body.gender_id;
            const address_id = req.body.address_id;
            client.query(`INSERT INTO patients (first_name, last_name, age, dob, email, phone_number, gender_id, address_id) VALUES 
...
router.get('/patients/:id/edit', isResource, isAuthenticated, function(req, res) {
    if (req.user.level === 1){
        client.query(`SELECT id, first_name, last_name, age, dob, email, phone_number, gender_id, address_id FROM patients WHERE id = '${req.params.id}'`,
            function (error, results, fields) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.put('/patients/:id', isResource, isAuthenticated, function(req, res, next) {
    ...
    if (!errors.isEmpty()) {
        ...
        const gender_id = req.body.gender_id;
        const address_id = req.body.address_id;
        client.query(`UPDATE patients SET first_name = '${first_name}', last_name = '${last_name}', age = '${age}', dob = '${dob}',
            email = '${email}', phone_number = '${phone_number}', gender_id = '${gender_id}', address_id = '${address_id}' WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
...
router.delete('/patients/:id', isResource, isAuthenticated, function(req, res, next) {
    if (req.user.level === 1){
        client.query(`DELETE FROM patients WHERE id = '${req.params.id}'`, function (error, results, fields) {
            // error will be an Error if one occurred during the query
            // results will contain the results of the query
            // fields will contain information about the returned results fields (if any)
            if (error) {
...
router.get('/procedures', function(req, res, next) {
    if (req.isAuthenticated()) {
        client.query('SELECT * FROM procedures ORDER BY date_created DESC; SELECT count(*) as count FROM procedures',
            function (error, results, fields) {
                if (error) {
                    throw error;
                }
...
router.post('/procedures', isAuthenticated, function(req, res, next) {
    ...
    , (req, res) => {
        ...
        if (!errors.isEmpty()) {
            ...
            const description = req.body.description;
            const cost = req.body.cost;
            client.query(`INSERT INTO procedures (name, description, cost) VALUES ('${name}','${description}','${cost}')`,
                function (error, results) {
                    // error will be an Error if one occurred during the query
                    // results will contain the results of the query
                    // fields will contain information about the returned results fields (if any)
...
router.get('/procedures/:id/edit', isResource, isAuthenticated, function(req, res) {
    if (req.user.level === 1){
        client.query(`SELECT id, name, description, cost FROM procedures WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.put('/procedures/:id', isResource, isAuthenticated, function(req, res, next) {
    ...
    if (!errors.isEmpty()) {
        ...
        const description = req.body.description;
        const cost = req.body.cost;
        client.query(`UPDATE procedures SET name = '${name}', description = '${description}', cost = '${cost}' WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.delete('/procedures/:id', isResource, isAuthenticated, function(req, res, next) {
    if (req.user.level === 1){
        client.query(`DELETE FROM procedures WHERE id = '${req.params.id}'`, function (error, results) {
            // error will be an Error if one occurred during the query
            // results will contain the results of the query
            // fields will contain information about the returned results fields (if any)
            if (error) {
...
router.get('/visits', function(req, res, next) {
    if (req.isAuthenticated()) {
        client.query('SELECT * FROM visits ORDER BY date_created DESC; SELECT count(*) as count FROM visits',
            function (error, results, fields) {
                if (error) {
                    throw error;
                }
...
router.post('/visits', isAuthenticated, function(req, res, next) {
    ...
    , (req, res) => {
        ...
        if (!errors.isEmpty()) {
            ...
            client.query(`INSERT INTO visits (patient_id, doctor_id) VALUES ('${patient_id}','${doctor_id}')`,
                function (error, results) {
                    // error will be an Error if one occurred during the query
                    // results will contain the results of the query
                    // fields will contain information about the returned results fields (if any)
...
router.get('/visits/:id/edit', isResource, isAuthenticated, function(req, res) {
    if (req.user.level === 1){
        client.query(`SELECT id, patient_id, doctor_id FROM visits WHERE id = '${req.params.id}'`,
            function (error, results, fields) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.put('/visits/:id', isResource, isAuthenticated, function(req, res, next) {
    ...
    if (!errors.isEmpty()) {
        ...
        const patient_id = req.body.patient_id;
        const doctor_id = req.body.doctor_id;
        client.query(`UPDATE visits SET patient_id = '${patient_id}', doctor_id = '${doctor_id}' WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.delete('/visits/:id', isResource, isAuthenticated, function(req, res, next) {
    if (req.user.level === 1){
        client.query(`DELETE FROM visits WHERE id = '${req.params.id}'`, function (error, results, fields) {
            // error will be an Error if one occurred during the query
            // results will contain the results of the query
            // fields will contain information about the returned results fields (if any)
            if (error) {
...
router.get('/visitsmedications', function(req, res, next) {
    if (req.isAuthenticated()) {
        client.query('SELECT * FROM visitsmedications ORDER BY date_created DESC; SELECT count(*) as count FROM visitsmedications',
            function (error, results, fields) {
                if (error) {
                    throw error;
                }
...
router.post('/visitsmedications', isAuthenticated, function(req, res, next) {
    ...
    , (req, res) => {
        ...
        if (!errors.isEmpty()) {
            ...
            client.query(`INSERT INTO visitsmedications (visit_id, medication_id) VALUES ('${visit_id}','${medication_id}')`,
                function (error, results) {
                    // error will be an Error if one occurred during the query
                    // results will contain the results of the query
                    // fields will contain information about the returned results fields (if any)
...
router.get('/visitsmedications/:id/edit', isResource, isAuthenticated, function(req, res) {
    if (req.user.level === 1){
        client.query(`SELECT id, visit_id, medication_id FROM visitsmedications WHERE id = '${req.params.id}'`,
            function (error, results, fields) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.put('/visitsmedications/:id', isResource, isAuthenticated, function(req, res, next) {
    ...
    if (!errors.isEmpty()) {
        ...
        const visit_id = req.body.visit_id;
        const medication_id = req.body.medication_id;
        client.query(`UPDATE visitsmedications SET visit_id = '${visit_id}', medication_id = '${medication_id}' WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.delete('/visitsmedications/:id', isResource, isAuthenticated, function(req, res, next) {
    if (req.user.level === 1){
        client.query(`DELETE FROM visitsmedications WHERE id = '${req.params.id}'`, function (error, results) {
            // error will be an Error if one occurred during the query
            // results will contain the results of the query
            // fields will contain information about the returned results fields (if any)
            if (error) {
...
router.get('/visitsprocedures', function(req, res, next) {
    if (req.isAuthenticated()) {
        client.query('SELECT * FROM visitsprocedures ORDER BY date_created DESC; SELECT count(*) as count FROM visitsprocedures',
            function (error, results, fields) {
                if (error) {
                    throw error;
                }
...
router.post('/visitsprocedures', isAuthenticated, function(req, res, next) {
    ...
    , (req, res) => {
        ...
        if (!errors.isEmpty()) {
            ...
            client.query(`INSERT INTO visitsprocedures (visit_id, procedure_id) VALUES ('${visit_id}','${procedure_id}')`,
                function (error, results) {
                    // error will be an Error if one occurred during the query
                    // results will contain the results of the query
                    // fields will contain information about the returned results fields (if any)
...
router.get('/visitsprocedures/:id/edit', isResource, isAuthenticated, function(req, res) {
    if (req.user.level === 1){
        client.query(`SELECT id, visit_id, procedure_id FROM visitsprocedures WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.put('/visitsprocedures/:id', isResource, isAuthenticated, function(req, res, next) {
    ...
    if (!errors.isEmpty()) {
        ...
        const visit_id = req.body.visit_id;
        const procedure_id = req.body.procedure_id;
        client.query(`UPDATE visitsprocedures SET visit_id = '${visit_id}', procedure_id = '${procedure_id}' WHERE id = '${req.params.id}'`,
            function (error, results) {
                // error will be an Error if one occurred during the query
                // results will contain the results of the query
                // fields will contain information about the returned results fields (if any)
...
router.delete('/visitsprocedures/:id', isResource, isAuthenticated, function(req, res, next) {
    if (req.user.level === 1){
        client.query(`DELETE FROM visitsprocedures WHERE id = '${req.params.id}'`, function (error, results, fields) {
            // error will be an Error if one occurred during the query
            // results will contain the results of the query
            // fields will contain information about the returned results fields (if any)
            if (error) {
